...
| 派生ビューは文章生成に頼らない | 設計レビュー観点・セキュリティ注意・隠れコスト・事例検索は、既存データからルールベースで機械的に組み立て |

## 主な機能

- サービス図鑑 — カテゴリ分類・キーワード検索・絞り込み（無料枠/お気に入り）。詳細では用途／不適合な場面／随伴サービス／採用判断／運用／一次体験ノート（出典つき）／共通ルールへのリンクを表示。
